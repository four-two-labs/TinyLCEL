...
def test_str_output_parser_parse_failure(
    parser: StrOutputParser, invalid_input: Any, expected_exception: Type[BaseException], match_message: str
) -> None:
...
def test_json_output_parser_parse_failure_str_parser(
    json_parser: JsonOutputParser, invalid_input: Any, expected_exception: Type[BaseException], match_message: str
) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        (
            '{"name": "Test", }',
            r'^Failed to parse extracted string as JSON: Expecting property name enclosed in double quotes',
        ),
        (
            "{'key': 'value'}",
            r'^Failed to parse extracted string as JSON: Expecting property name enclosed in double quotes',
        ),
        ('[1, 2,]', r'^Failed to parse extracted string as JSON: Expecting value'),
...
def test_json_output_parser_parse_failure_json_decode(
    json_parser: JsonOutputParser, bad_json_content: str, match_message: str
) -> None:
    """Test that JSONDecodeError is caught and wrapped in ParseError."""
    message = AIMessage(content=bad_json_content)
    with pytest.raises(ParseError, match=match_message) as exc_info:
...
async def test_json_output_parser_aparse_failure_json_decode(json_parser: JsonOutputParser) -> None:
    ...
    message = AIMessage(content=bad_json_content)
    with pytest.raises(ParseError, match=r'^Failed to parse extracted string as JSON') as exc_info:
...
def test_json_output_parser_invoke_failure_json_decode(json_parser: JsonOutputParser) -> None:
    ...
    with pytest.raises(ParseError, match=r'^Failed to parse extracted string as JSON'):
...
def test_yaml_output_parser_parse_failure_str_parser(
    yaml_parser: YamlOutputParser, invalid_input: Any, expected_exception: Type[BaseException], match_message: str
) -> None:
...
def test_yaml_output_parser_parse_failure_yaml_error(
    yaml_parser: YamlOutputParser, bad_yaml_content: str, match_message: str
) -> None:
...
async def test_yaml_output_parser_aparse_failure_yaml_error(yaml_parser: YamlOutputParser) -> None:
    ...
    with pytest.raises(ParseError, match=r'^Failed to parse extracted string as YAML') as exc_info:
...
def test_yaml_output_parser_invoke_failure_yaml_error(yaml_parser: YamlOutputParser) -> None:
    ...
    with pytest.raises(ParseError, match=r'^Failed to parse extracted string as YAML'):
