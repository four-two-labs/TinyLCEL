...
from typing import Dict  # Shortened imports, removed Tuple
from typing import List  # Shortened imports, removed Tuple
...
class TestChatPromptTemplateInitialization:
    ...
    def test_init_invalid_type_in_list(self) -> None:
        with pytest.raises(
            TypeError,
            match="Item at index 1 in 'message_templates' must be a \\(role: str, template: str\\) tuple or a BaseMessage instance",
        ):
...
class TestChatPromptTemplateFormatting:
    def test_format_messages_success(
    ...
    def test_format_messages_with_complex_vars(
    ...
    def test_format_messages_missing_input_variable_error(
        self, basic_template_tuples: List[MessageTemplateType]
    ) -> None:
        prompt = ChatPromptTemplate(basic_template_tuples)  # type: ignore[arg-type]
        with pytest.raises(
            KeyError, match=r"Missing input variables: \['bot_name'\]. Provided variables: \['user_name'\]"
        ):
    ...
    def test_format_messages_template_string_key_error(self) -> None:
        ...
        with pytest.raises(
            KeyError, match=r"Missing input variables: \['message'\]. Provided variables: \['code', 'name'\]"
        ):
        ...
        with pytest.raises(
            KeyError,
            match=r"A variable required by the template string was not found in the provided arguments. Missing key: 'value'. Template: 'Hello {name}, you have {message}. Your code is {code\[value\]}'",
        ):
            prompt.format_messages(name='User', message='A message', code={'actual_value_is_missing': 1})

    def test_format_messages_value_error_for_object_attribute_format_issue(
        ...
        with pytest.raises(
            ValueError,
            match="Error formatting template.*AttributeError: .*DummyUser.* object has no attribute 'non_existent_attr'",
        ):
...
class TestChatPromptTemplateRunnableMethods:
    ...
    @pytest.mark.asyncio
    async def test_abatch_return_exceptions(
