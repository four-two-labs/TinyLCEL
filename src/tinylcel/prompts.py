...
type MessageTemplateType = Union[tuple[str, str], BaseMessage]
...
class ChatPromptTemplate(RunnableBase[dict[str, Any], list[BaseMessage]]):
    ...
    def input_variables(self) -> list[str]:
        ...
        """
        return sorted(list(self._input_variables_set))
    ...
    def format_messages(self, **kwargs: Any) -> list[BaseMessage]:
        ...
        if missing_vars:
            raise KeyError(
                f'Missing input variables: {sorted(list(missing_vars))}. '
                f'Provided variables: {sorted(list(kwargs.keys()))}'
            )

        return [
            format_message(role_str, template_str) 
            for role_str, template_str in self.message_templates
        ]
    ...
    def from_messages(
        cls,
        messages: list[MessageTemplateType]
    ) -> 'ChatPromptTemplate':
        """
